...
$$Xβ=ln(2.89* (viral load-4.0)+lna(RRcircumcised} )*circumcised+lna(RR_condom )*condom+ ln(RR_age )* age_base+lna(RR_sti )*sti_status_sus$$
where circumcised is an indicator variable for the circumcision status of the male, condom is an indicator variable for condom use for the male, viral load is the log10 viral load of infected agent, and age_base is the number of decades over 35 the susceptible agent is: (age-35)/10; and RR_x is the risk factor. 

For MSM partners,

$$Xβ=lna〖(RR_VL )*〗 (viral load-4.0)+lna(RR_insertive )*insertive+lna(RR_receptive )*receptive+lna((RR_circumcised ))*circumcised+lna(RR_condom )*condom+
lna(RR_sti )*sti_status_su$$s
