...
param_list=list(
  ...
  #Vaccine parameters ----------------------------------------- #
  start_vacc_campaign = (10*365):(50*365),    # vaccination occurs every timestep beginning at start of year 11
  ...
  perc_virus_vaccine_sens = 0.5,     #proportion of initial marks that are sensitive to vaccine 
  vacc_trans_prob_decrease =0.8,     #proportion (percentage) decrease in trans probs due to vaccine for vaccine model "1" (baseline vaccine model),
  mu_daily_mutate_rate1    = 1/(365*5),  #daily probability of mu=1 -> mu=0 (model 1b)
...
evoparams$vacc_model_id <- "model_1b"
...
#assign model names
model_name = paste("new_vaccine_test1b_example1.RData",sep="")
#save model
