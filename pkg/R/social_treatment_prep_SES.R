...
social_treatement_prep_SES<-function(dat,at){
  ...
  if(at<3){return(dat)}
   ...
   #---------------------------------------------
   # track partnerships with recently tested partners
   ...
   #---------------------------------------------
   # track partnerships with supressed partners
  ...
  #       is.na(dat$pop$diag_status))                                                       # AND does not have diagnosed HIV
   ...
   prep_ix <-  which((
        ...
        dat$pop$have_disc_partner == 1  & dat$pop$have_suppressed_partner !=1)  &  # current partner has disclosed HIV+ status & not suppressed
        is.na(dat$pop$diag_status) &                                        # AND does not have diagnosed HIV
        dat$pop$no_partners_past_prep >= dat$param$min_past_partners_prep & # AND has had at least 1 partner in last 6 months OR
        ((dat$pop$no_partners_now_prep==1 & dat$pop$partner_recent_test==1)==F)  # AND not in monogamous recently tested partnership
          )
