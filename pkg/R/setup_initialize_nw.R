...
#' @details
#' Creates initial network and sets network attributes "age","sqrt_age", "sex",
#' "role", and "att1" for each node (agent) on the network, which is required
#' for estimation of networks using these attributes. Wrapper function for
#' "network.initialize" and "set.vertex.attribute" functions from "network"
#' package. If additional attributes are desired, they can be added following
#' the template within the function.
#'
#' @examples
...
setup_initialize_network <- function(params) {
  ...
  nw <- network::set.vertex.attribute(x = nw, attrname = "id", value = id_vec)


  if (params$model_sex != "msm") {
    sex_vec <- sample(c("m", "f"), params$initial_pop,
                      prob = c(0.5, 0.5), replace = TRUE)
    nw <- network::set.vertex.attribute(x = nw, attrname = "sex",
                                        value = sex_vec)
  } else {
    network::set.vertex.attribute(x = nw, attrname = "sex", value = "m")
  }
  ...
  nw <- network::set.vertex.attribute(x = nw, attrname = "age",
                                      value = age_vec)
  nw <- network::set.vertex.attribute(x = nw, attrname = "sqrt_age",
                                      value = sqrt(age_vec))
