...
{  
  ...
  if(dat$param$VL_Function=="aim3"){
    ...
    infectees <- which(dat$pop$CD4 < 5 & dat$pop$Status==1  & dat$pop$treated!=1)
  ...
  for(cd4_cat in 1:3){ #loop over cd4 cats (1,2,3)
    if(age_cat==1){
      infectee_subset <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                 dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] < 25)
    }
    if(age_cat==2){
      infectee_subset  <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                 dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] >= 25 & dat$pop$age[infectees] < 35)
    }
    if(age_cat==3){
      infectee_subset  <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                  dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] >= 35 & dat$pop$age[infectees] < 45)
    }
    if(age_cat==4){
      infectee_subset  <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                  dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] >= 45 & dat$pop$age[infectees] < 55)
    }
    if(age_cat==5){
      infectee_subset  <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                  dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] >= 55 & dat$pop$age[infectees] < 65)
    }
    if(age_cat==6){                        
      infectee_subset  <- which(dat$pop$CD4[infectees] ==cd4_cat & dat$pop$Status[infectees]==1  & 
                                 dat$pop$treated[infectees]!=1 & dat$pop$age[infectees] >= 65)
    }
    
    if(length(infectee_subset)>0){
      cd4_times_index <-cbind(dat$pop$spvl_cat[infectees][infectee_subset],rep(cd4_cat,length(infectee_subset)))
  ...
  infectee_subset <- which(dat$pop$CD4[infectees]==4)
  if(length(infectee_subset)>0){
    cd4_times_index <- cbind(dat$pop$spvl_cat[infectees][infectee_subset],rep(4,length(infectee_subset)))
    cd4_threshold_time[infectee_subset] <-   dat$pop$CD4_time[infectees][infectee_subset] > (dat$param$CD4_lookup[cd4_times_index]*365)
  } 
  ...
  {    
    ...
    if(dat$param$aids_death_model=="cd4"){
      if(any(dat$pop$CD4[infected_and_cd4_increment]==4)){
        ...
        dat$pop$start_aids_cd4[final_index] <- at
      }}
