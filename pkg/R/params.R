...
param_evonet <- function(
  ...
  start_timestep = 1, # parameter for EpiModel, should be "1" if new simulation
                      # if re-starting simulation, value should be "n_steps+1" (n_steps from original sim.)
  ...
  Heritability          = 0.36,     # desired population heritability estimate is 0.36 (from Fraser; should vary by population).
                                    # This value, in conjunction with the default mutation variance, seems to yield a stable
                                    # population heritability of 3.6.  But user should always beware!
  ...
  Therapy_Type = 1,# Therapy_Type = 1 eans three individual pills
                   # Therapy_Type = 2 means that drugs 1 and 2 are contained within a single pill,
                   # Therapy_Type = 3 means that all three drugs are contained within a single pill
  ...
  baseline_input_exp_growth = 0.007, # Used with "exponential_growth"  MUST BE SCALED BY HAND TO GET A STABLE AGE DISTRIBUTIONbirth_model              = "poisson_birth_numbers"
  ...
  risk_group_prep_adherance_prob = c(0.21,0.07,0.10,0.62), #PrEP adherance in risk group: proportion in each adherance category,
                                                           # corresponding to 0,<2,2-3,4+ doses/week, after Jenness et al JID 2016
  main_group_prep_adherance_prob = c(0.21,0.07,0.10,0.62), #PrEP adherance in main group: proportion in each adherance category,
                                                           # corresponding to 0,<2,2-3,4+ doses/week, after Jenness et al JID 2016
  ...
  p$poisson_birth_lambda     = (p$initial_pop/100) * p$poisson_birth_lambda_base
