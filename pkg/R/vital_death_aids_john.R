...
#' @export
vital_death_aids_john <-function(dat,at)
{
  ...
  List_PostAcute <- ( timeIndex-dat$pop$Time_Inf_Adj ) > dat$param$t_acute
  ...
  if (Cond1_Gamma_Death) {  
    ...
    List_ProgressedToAIDS_UnderGammaDeath <- ( timeIndex - dat$pop$Time_Inf_Adj ) > ( dat$pop$RandomTimeToAIDS +dat$param$time_in_aids )
    # Notes: 
