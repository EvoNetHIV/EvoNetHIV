...
social_increased_testing_selected_groups_module <- function(dat, at){   
  ...
  for (j in 1:num_care_strategies) {
     ...
     not_curr_enhanced <- dat$attr$Status >= 0 &  dat$attr$diag_status %in% c(0,NA) & dat$attr$enhanced_testing %in% c(0,NA)
   ...
   if (scale_up_strategy[j] == "recent_sex_known_hiv_pos")   eligible_enhancement <- not_curr_enhanced & (at - dat$attr$time_hiv_sex_act <= 90)
    ...
    if (length(which(eligible_enhancement==TRUE)) >= 1) {
      ...
      if (dat$param$testing_limit == "percent_agents_minus_diagnosed") {
        upper_limit_enhanced_testing <- round(  length(which(dat$attr$Status >= 0))*dat$param$prob_enhanced_testing_after_campaign 
                                              - length(which(dat$attr$diag_status==1)) )
      }
      # Determine number who are already being tested more frequently 
      current_enhanced_testing <- length(which(dat$attr$Status >= 0 & dat$attr$diag_status==1))
     ...
     if (num_eligible_enhancement <= max_enhanced_testing) {
      ...
      last_ever_enhanced_testing <- dat$attr$ever_enhanced_testing
      dat$attr$ever_enhanced_testing[selected] <- 1
      #if (length(setdiff(which(dat$attr$ever_enhanced_testing==1),which(last_ever_enhanced_testing==1)))>0) {
      #  cat("\nsetdiff: ",setdiff(which(dat$attr$ever_enhanced_testing==1),which(last_ever_enhanced_testing==1)),"\n")
      #}
