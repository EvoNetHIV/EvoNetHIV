...
viral_update_delayed_rebound <- function(dat, at) {
  ...
  #    dat$attr$Time_Inf_Adj   (time person was infected after adjusting for times on therapy)                                                  #
  ###########################################################################################################
  ...
  acute       <-  timestep <= (dat$attr$Time_Inf_Adj+dat$param$t_acute) 
  acute_exp <-  timestep <= (dat$attr$Time_Inf_Adj+dat$param$t_peak)
  acute_ph1 <- (acute & !acute_exp & 
                  ((timestep-dat$attr$Time_Inf_Adj) <= dat$param$t_acute_phase2) )
  
  if(dat$param$aids_death_model=="Gamma_Death"){
    aids_start<-  timestep > (dat$attr$Time_Inf_Adj + dat$attr$RandomTimeToAIDS)
  }else{
  ...
  if(length(acute_exp_ix)>0){
    inf_duration <- (timestep-dat$attr$Time_Inf_Adj[acute_exp_ix])
    dat$attr$V[acute_exp_ix] <- dat$param$V0 * exp(dat$attr$r0[acute_exp_ix]*inf_duration)
  }  
  ...
  if(length(acute_ph1_ix)>0) {
    ...
    #------
    #new 3/2/16
    inf_duration <- (timestep-dat$attr$Time_Inf_Adj[acute_ph1_ix])
  ...
  if(length(acute_ph2_ix)>0) {
    ...
    #new 3/2/16
    ix <- acute_ph2_ix
    inf_duration <- (timestep-dat$attr$Time_Inf_Adj[ix])
  ...
  if(length(post_acute_ix)>0){
    ...
    dat$attr$V[post_acute_ix] <- (tempVss * exp(dat$attr$s[post_acute_ix] *
                                                 (timestep-dat$param$t_acute-dat$attr$Time_Inf_Adj[post_acute_ix])/365.0))
  }
