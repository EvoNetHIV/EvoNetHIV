...
social_treatment_module_multiple_criteria <- function(dat, at)
{
 ...
 # Outputs:
  ...
  if (at >= dat$param$start_treat_before_big_campaign &
    ...
    new_eligible_tx <- which(dat$pop$Status == 1 & dat$pop$eligible_care == 1 & dat$pop$diag_status == 1 & dat$pop$treated == 0)
  ...
  for (j in 1:num_tx_strategies) {
    ...
    not_curr_tx <- dat$pop$Status == 1 & dat$pop$eligible_care == 1 & dat$pop$diag_status == 1 & dat$pop$treated == 0
    eligible_tx <- NULL
    ...
    if (!is.null(not_curr_tx)) {
      ...
      if (tx_strategy[j] == "diag10yrs")  eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 10*365)
      if (tx_strategy[j] == "diag8rsyr")  eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 8*365)
      if (tx_strategy[j] == "diag6yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 6*365)
      if (tx_strategy[j] == "diag5yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 5*365)
      if (tx_strategy[j] == "diga4yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 4*365)
      if (tx_strategy[j] == "diag3yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 3*365)
      if (tx_strategy[j] == "diag2yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 2*365)
      if (tx_strategy[j] == "diag1yrs")   eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 1*365)
      if (tx_strategy[j] == "diag0.5yrs") eligible_tx <- not_curr_tx & (at - dat$pop$diag_time > 0.5*365)
      ...
      if (tx_strategy[j] == "riskgroup_over2" ) eligible_tx <- not_curr_tx & dat$pop$att1 >= 2
