...
{  
  ...
  if(dat$param$VL_Function=="aim3"){
    ...
    infectees <- which(dat$pop$CD4 < 5 & dat$pop$Status==1  &
                         dat$pop$treated!=1 & dat$pop$vaccinated!=1)
  }
  ...
  ix <- which(dat$pop$CD4[infectees]==4)
  if(length(ix)>0){
    #if(any(dat$pop$spvl_cat[infectees][ix]<=5)){browser()}
    cd4_times_index <- cbind(dat$pop$spvl_cat[infectees][ix],rep(4,length(ix)))
    cd4_threshold_time[ix] <-   dat$pop$CD4_time[infectees][ix] > (dat$param$CD4_lookup[cd4_times_index]*365)
  } 
