void update_drug_concs_pill_combos_stockout(double h, double Time,double time_0,
  ...
  if (Therapy_Type == TripleDrugCombo) {  // Drugs 1, 2 and 3 are taken togetehr
   ...
   if ( (Time < StopDrug4) || (Time >= RestartDrug4)) {  // CHANGED THIS LINE TO MODEL TAIL COVERING WITH DRUG 4
                                                         // Assume that drug 4 is not given before or after the stockout
                                                         // time specific for drug 4. Note the the parameters are badly
                                                         // named in this variant.
